...
A two-sample t-test is used in this assessment to compare measured soil carbon results across the two strata. The null hypothesis is tested that the true difference in means between the two groups is equal to 0.
...
df_elig_comb = df_elig_labelled %>%
   ...
   left_join(df_soil_sites, by = c("Landscape_Name", "Group")) %>%
    mutate(Soil_Sites = Soil_Sites*coef_sec.axis) %>%
...
df_elig_comb %>%
  ...
  scale_x_continuous(labels=function(x) format(x, big.mark = ",", decimal.mark = ".", scientific = FALSE),
                     name = "Land cover extent (hectares)", 
...
with(df_soc, shapiro.test(SOC_log10[Group == "Non-Woody"])) # p-value = 0.1072
...
var.test(SOC_log10 ~ Group, data = df_soc) # p-value = 0.01134
...
res <- t.test(SOC_log10 ~ Group, data = df_soc, var.equal = FALSE) # p-value = 0.2302
res
